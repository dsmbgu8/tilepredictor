...
def extract_patches_2d(image, patch_size, max_patches=None, random_state=None,
                       return_index=False, writeable=False, randomize=True):
    """
    ...
    extracted_patches = extract_patches(image, patch_shape=patch_shape,
                                        extraction_step=1, writeable=False)
    ...
    if randomize:
        ...
        i_s = rng.randint(h_max, size=max_patches)
        j_s = rng.randint(w_max, size=max_patches)
        patches = extracted_patches[i_s, j_s, 0]
    elif max_patches == n_patches:
        patches = extracted_patches[:, :, 0]
        if return_index:
            i_s,j_s = map(np.ravel,np.meshgrid(np.arange(h_max, dtype=np.int),
                                               np.arange(w_max, dtype=np.int)))

    else:
        # return the first max_patches patches without randomization
        i_s,j_s = map(np.ravel,np.meshgrid(np.arange(h_max, dtype=np.int),
                                           np.arange(w_max, dtype=np.int)))
        patches = extracted_patches[i_s[:max_patches],j_s[:max_patches], 0]
    
    
    # remove the color dimension if useless
    if patches.shape[-1] == 1:
        patches = patches.reshape(max_patches, p_h, p_w)
    else:
        patches = patches.reshape(max_patches, p_h, p_w, n_colors)
...
if __name__ == '__main__':
    ...
    #        [ 8,  9, 10, 11],
    #        [12, 13, 14, 15]])
    patches,indices = extract_patches_2d(input_image, (2, 2), return_index=True)
    print(patches.shape)
    # (9, 2, 2)
    print(indices.shape)
    # (9,2)
